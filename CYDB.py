import sqlite3
import time
from data_change import split_dict, cutting_dict, dict_to_table, tuple_to_str, tuple_to_dict
...
def create_table(table_name, key_dict):
    """
    创建一个表
    :param table_name:表名
    :param key_dict: { 'key':'类型','key2':'类型' }，类型有(pk,'')
    :return:无
    """
    command = 'create table {} ({})'.format(
        table_name, dict_to_table(key_dict))
    return call(command)
